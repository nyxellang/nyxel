...
class TestBytecodeDesign:

    def test_opcode_enum_has_expected_instructions(self):
        from nyxel.bytecode import Op
        required = [
            "LOAD_CONST", "LOAD", "STORE", "DEFINE",
            "BIN_OP", "JUMP", "JUMP_IF_FALSE",
            "CALL", "RETURN",
            "BUILD_LIST", "BUILD_DICT",
            "TRY_START", "TRY_END",
            "BRING",
        ]
        op_names = [o.name for o in Op]
        for name in required:
            assert name in op_names, f"Missing opcode: {name}"

    def test_code_object_add_const(self):
        from nyxel.bytecode import CodeObject
        co = CodeObject("test")
        idx1 = co.add_const(42)
        idx2 = co.add_const("hello")
        idx3 = co.add_const(42)   # should reuse
        assert idx1 == 0
        assert idx2 == 1
        assert idx3 == 0           # same as first 42

    def test_code_object_emit_and_disassemble(self):
        from nyxel.bytecode import CodeObject, Op
        co = CodeObject("test")
        i1 = co.add_const(5)
        co.emit(Op.LOAD_CONST, i1)
        n1 = co.add_name("x")
        co.emit(Op.STORE, n1)
        listing = co.disassemble()
        assert "LOAD_CONST" in listing
        assert "STORE" in listing

    def test_code_object_patch(self):
        from nyxel.bytecode import CodeObject, Op
        co = CodeObject("test")
        jump_idx = co.emit(Op.JUMP, 0)   # placeholder
        co.emit(Op.LOAD_NONE)
        co.patch(jump_idx, 99)
        assert co.instructions[jump_idx].arg == 99

    def test_compiler_runs_without_error(self):
        """Compiler should not crash on basic programs."""
        from nyxel.bytecode import Compiler
        code = "let x = 3 + 4\nif x > 5:\n    let y = x * 2\n"
        toks  = lex(code)
        stmts = Parser(toks).parse()
        compiler = Compiler()
        co = compiler.compile(stmts)
        assert co is not None
        assert len(co.instructions) > 0

    def test_vm_executes_simple_arithmetic(self):
        """VM should evaluate  3 + 4  correctly."""
        from nyxel.bytecode import CodeObject, Op, VM, _BIN_OPS
        co = CodeObject("test")
        i3 = co.add_const(3); i4 = co.add_const(4)
        co.emit(Op.LOAD_CONST, i3)
        co.emit(Op.LOAD_CONST, i4)
        co.emit(Op.BIN_OP, _BIN_OPS.index("+"))
        ix = co.add_name("result")
        co.emit(Op.STORE, ix)
        vm = VM(co)
        vm.run()
        assert vm._globals.get("result") == 7
